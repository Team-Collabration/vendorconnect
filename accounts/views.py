    ...
    if request.method == "POST":
        ...
        try:
            send_mail(
                ...
                from_email="vendorconnectweb@gmail.com",
                recipient_list=[email],
                fail_silently=False,
            )
            return JsonResponse({"message": f"OTP sent successfully to {email}"})
...
def view_profile(request):
    ...
    context = {
        'user': user,
        'supplier': supplier  # matches the template
    }
...
def create_payment(request):
    if request.method == "POST":
        account_type = request.POST.get("account_type")  # vendor/supplier
        if account_type == "vendor":
            amount = 29900
        elif account_type == "supplier":
            amount = 49900
        ...
        try:
            ...
            request.session["signup_data"]["account_type"] = account_type
            return JsonResponse({
                "order_id": order["id"],
                "amount": amount,
...
def payment_success(request):
    ...
    if request.method == "POST":
        ...
        if not User.objects.filter(email=signup_data["email"]).exists():
            ...
            if account_type == "vendor":
                VendorProfile.objects.create(user=user, business_name=signup_data["username"])
            else:
                SupplierProfile.objects.create(user=user, business_name=signup_data["username"])

        # Determine redirect URL based on account type
        if account_type == "vendor":
            redirect_url = "/api/accounts/vendor/dashboard/"
        else:
            redirect_url = "/api/accounts/suppliers/dashboard/"
